...
//					roundType = RoundType.STRIKE;
	...
	private int getScoreFromSimpleRound() throws GameException {
		...
		if (!lastRound && newScore > BowlingGame.maxShotValue) {
			throw new ScoreExceedsAllowedValueException(this, "Score " + newScore + " in round " + getNumber()
	...
	/**
	 * Add aditional bowling score in string format
	 * 
	 * @throws GameException In case score does not fit exepected values
	 */
	public void addScore(String score) throws GameException {
		...
		if (pointsSize > maxShoots)
			throw new ExceedMaximumNumberOfRollsException(this,
					"User can't do more than " + maxShoots + " rolls in round " + getNumber());
